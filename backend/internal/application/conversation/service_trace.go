...
// buildReasoningPassbackProcessTrace 构造"推理回传"阶段的处理过程追踪，
// MessageCount 供前端展示本轮回传的历史推理消息条数。
func buildReasoningPassbackProcessTrace(messageCount int) (string, string, *tracePayload) {
	detail := fmt.Sprintf("已向模型回传 %d 条历史推理内容。", messageCount)
	stage := traceStage{Kind: processTraceKindReasoningPassback, Status: processTraceStatusCompleted, MessageCount: messageCount}
	return "", formatTraceStep("推理回传", detail), &tracePayload{TraceStage: &stage}
}
