...
func TestConversationFallbackTitleUsesUnifiedLimit(t *testing.T) {
	if conversationFallbackTitleMaxRunes != 30 {
		t.Fatalf("expected fallback title limit to stay unified at 30, got %d", conversationFallbackTitleMaxRunes)
	}

	got := conversationTitleFromFirstUserMessage("0123456789abcdefghij0123456789abcdefghij01234")
	if got != "0123456789abcdefghij0123456789" {
		t.Fatalf("expected fallback title to truncate to 30 runes, got %q", got)
	}
}
